...
public class Prediction {
	...
	public static void main(String [] args) {
		...
		// in "COLLECTION_NAME"(a table name), find all documents(row) whose user_id equals to USER_ID, and put these documents in to FindIterable 
		FindIterable<Document> iterable = db.getCollection(COLLECTION_NAME).find(
				eq(USER_COLUMN, USER_ID));

		iterable.forEach(new Block<Document>() { // anonymous class
			@Override
		...
		// Construct mapper function, which is the same with above comment example. Why we need to build function in some what js code way?
		// because mongodb's mapReduce function ask we do like this way.......
		...
		for (User user : similarUsers) {
			...
			iterable.forEach(new Block<Document>() {
				...
				public void apply(final Document document) {
					...
					if (!previousItems.contains(item) && products.size() < 5 ){ // previousItems is target user's bought record. 5 means we recommend 5 items at most
